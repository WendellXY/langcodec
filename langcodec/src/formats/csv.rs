...
impl Parser for Vec<MultiLanguageCSVRecord> {
...
// Helper function to convert MultiLanguageCSVRecord to Vec<Resource>
pub fn multi_language_csv_to_resources(records: Vec<MultiLanguageCSVRecord>) -> Vec<Resource> {
    if records.is_empty() {
        return Vec::new();
    }
...
// Helper function to convert Vec<Resource> to MultiLanguageCSVRecord
pub fn resources_to_multi_language_csv(
    resources: &[Resource],
) -> Result<Vec<MultiLanguageCSVRecord>, Error> {
...
mod tests {
    ...
    fn test_parse_multi_language_csv() {
        ...
        let records = Vec::<MultiLanguageCSVRecord>::from_reader(Cursor::new(csv_content)).unwrap();
        assert_eq!(records.len(), 2);
    ...
    fn test_multi_language_csv_to_resources() {
        let csv_content = "key,en,cn\nhello,Hello,你好\nbye,Goodbye,再见\n";
        let records = Vec::<MultiLanguageCSVRecord>::from_reader(Cursor::new(csv_content)).unwrap();
        let resources = multi_language_csv_to_resources(records);
